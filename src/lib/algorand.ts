...
export async function getLogicFromTransaction(
  addr: string
): Promise<LogicSigAccount> {
  const indexer = getIndexer()
  const txns = await indexer.searchForTransactions().address(addr).do()

  for (let tidx in txns.transactions) {
    const txn = txns.transactions[tidx]
    if (txn.sender == addr) {
      const program_bytes = new Uint8Array(
        Buffer.from(txn.signature.logicsig.logic, 'base64')
      )
...
export async function isOptedIntoApp(address: string): Promise<boolean> {
  ...
  const optedIn = result['apps-local-state'].find((r) => {
    return r.id == ps.application.app_id
  })
...
export async function isOptedIntoAsset(
  ...
  const client = getAlgodClient()
  const result = await client.accountInformation(address).do()
  const optedIn = result['assets'].find((r) => {
    return r['asset-id'] == idx
  })
  return optedIn !== undefined
}
...
export async function isListing(address: string): Promise<boolean> {
  const client = getAlgodClient()
  const result = await client.accountInformation(address).do()
  const hasPriceToken = result['assets'].find((r) => {
    return r['asset-id'] == ps.application.price_id
  })
...
export async function getListings(
  ...
  for (let bidx in balances.balances) {
    ...
    const list = await getListing(b.address)
    listings.push(list)
  }
...
export async function getPortfolio(addr: string): Promise<Portfolio> {
  ...
  for (let aidx in acct['apps-local-state']) {
    ...
    for (let kidx in als['key-value']) {
      ...
      lp.push(
        getListing(b64ToAddr(kv.key)).then((listing) => {
  ...
  for (let aidx in acct['assets']) {
    ...
    try {
      np.push(
        tryGetNFT(ass['asset-id']).then((nft) => {
...
export async function getHoldingsFromListingAddress(
  address: string
): Promise<Holdings> {
  const client = getAlgodClient()
  ...
  for (let aid in account.assets) {
    ...
    gets.push(
      getToken(asa['asset-id']).then(async (token) => {
        if (token.params.creator == ps.application.owner_addr)
          holdings.tags.push(TagToken.fromAsset(token))
